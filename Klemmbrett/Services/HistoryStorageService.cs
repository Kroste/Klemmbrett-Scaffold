...
public sealed class HistoryStorageService
{
    ...
    private sealed record StoredEntry(string Type, DateTimeOffset CapturedAt, string? Text, string? Hash);
        ...
        try
        {
            switch (s.Type)
            {
                case "text" when s.Text is not null:
                    return new TextClipboardEntry(s.Text, s.CapturedAt);
                case "image" when s.Hash is not null:
                    var path = ImagePath(s.Hash);
                    if (!File.Exists(path)) return null;
                    using (var fs = File.OpenRead(path))
                        return new ImageClipboardEntry(new Bitmap(fs), s.Hash, s.CapturedAt);
                default:
                    return null;
            }
        }
    ...
    public void SaveIndex(IReadOnlyList<IClipboardEntry> entries)
    {
        try
        {
            ...
            {
                TextClipboardEntry t => new StoredEntry("text", t.CapturedAt, t.Text, null),
                ImageClipboardEntry i => new StoredEntry("image", i.CapturedAt, null, i.ContentHash),
                _ => null
